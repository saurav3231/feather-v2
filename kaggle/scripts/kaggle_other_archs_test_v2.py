...
def _fake_other_benchmark() -> dict:
    ...
    for fam in families:
        base_loss = {"transformer": 2.69, "retnet": 2.56, "mamba": 2.63, "rwkv": 2.56}[
            fam
        ]
        for seed in [42, 7]:
...
def main() -> None:
    ...
    for row in board:
        print(
            f"  {row['family']}: loss={row['eval_loss']} cpu_tok_s={row['cpu_tok_s']:.2f} ram={row['ram_gb']:.1f}GB energy={row['energy_j_per_1k']:.2f}J"
        )

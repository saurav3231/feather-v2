...
def _load_real_wikitext(
    path: str = "feather-v1/wikitext-train-raw-v1.txt",
) -> list[str]:
    try:
...
def main() -> None:
    ...
    byte_ids = np.frombuffer(tokens_str.encode("utf-8"), dtype=np.uint8).astype(
        np.int64
    )
    ...
    print(
        f"Kernel: {kernel['binding']} {kernel['hypervector_dim']}-D {kernel['threads']} threads"
    )
    ...
    for step in range(min(5, len(data))):
        ...
        out = model.forward(x)
        logits = (
            np.asarray(out["final_output"], dtype=np.float64) @ model._logit_projection
        )
